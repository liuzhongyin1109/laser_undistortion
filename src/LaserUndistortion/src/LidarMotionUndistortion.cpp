...
public:
    ...
    void ScanCallBack(const sensor_msgs::LaserScanConstPtr& scan_msg)
    {
        ...
        } //将每一帧激光数据保存到vector向量中,后面可以考虑把原始点云时间戳加入容器中
        ...
        for(int i = 0; i < ranges.size();i++)
        {
            ...
            if(ranges[i] < 0.05 || std::isnan(ranges[i]) || std::isinf(ranges[i]))//滤波极小值以及inf或者ｎａｎ值等异常值
                continue;
            pcl::PointXYZRGB pt;
            ...
            visual_cloud_.push_back(pt); //将激光数据转出点云数据
        }
        std::cout << std::endl;
        //进行矫正
    ...
    /**
     * @name getLaserPose()
     * @brief 得到机器人在里程计坐标系中的位姿tf::Pose　即机器人位姿
    ...
    */
                      ...
                      tf::TransformListener * tf_)
    {
        ...
        try
        {
            ...
            tf_->transformPose("/odom", robot_pose, odom_pose);//返回dt时间戳下机器人位姿
        }
     ...
     *        激光雷达运动畸变去除分段函数;
     *        在此分段函数中，认为机器人是匀速运动；
     * @param frame_base_pose       标定完毕之后的基准坐标系,该值为ts时刻laser_link的位姿
                           ...
                           tf::TransformListener * tf_)
    {
        ...
        for(int i = 0; i < beamNumber; i++)
        {
            ...
            if(mid_time - start_time > interpolation_time_duration || (i == beamNumber - 1))
            {
                cnt++;
                //得到起点和终点的位姿
                //终点的位姿，５ms进行插值，获得中间点的机器人位姿（该位姿基于odom坐标系下，base_link的坐标）
